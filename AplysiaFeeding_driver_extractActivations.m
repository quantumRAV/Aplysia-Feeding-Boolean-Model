...
[BitingStruct,BitingTable] = extractActivation(aplysia,"Biting");
...
[SwallowStruct,SwallowTable] = extractActivation(aplysia,"Swallow");
...
[RejectStruct,RejectTable] = extractActivation(aplysia,"Reject")
...
%% save data from activation for slug bot:
save("ActivationVariables","BitingStruct","BitingTable", "SwallowStruct","SwallowTable","RejectStruct","RejectTable");
...
function [outStruct,outTable] = extractActivation(aplysia,tname)
    ...
    for i =1:length(activationNames)
        ...
        outStruct.(NeuralNames{i}+"_string_interp") = "bool " + NeuralNames{i} +"[" + string(length(outStruct.newT))+"] ={"+strjoin(string(outStruct.(NeuralNames{i}+"_interpl")), ' , ')+"};"; %interpolated neural activity...may not be 0 or 1
        %fprintf(fileID,NeuralNames{i}+"_interpl"+":"+outStruct.(NeuralNames{i}+"_interpl") +"\n\n\n");
        outStruct.(NeuralNames{i}+"_string") = "bool " + NeuralNames{i} +"[" + string(length(outStruct.t))+"] ={"+strjoin(string(aplysia.(NeuralNames{i})), ' , ')+"};"; %original neural activity
        fprintf(fileID,outStruct.(NeuralNames{i}+"_string") +"\n\n\n");
     
        ax1=subplot(length(activationNames),1,i);
        hold(ax1,'on');      
        plot(outStruct.t,aplysia.(activationNames{i}),'r-','DisplayName',"Activation of "+activationNames{i});
        plot(outStruct.t,aplysia.(forceNames{i}),'b-','DisplayName',"Tension of "+forceNames{i});
        plot(outStruct.t,aplysia.(NeuralNames{i}),'g-','DisplayName',"NeuralActivity of "+forceNames{i});
        legend();
        title(tname+activationNames{i});
